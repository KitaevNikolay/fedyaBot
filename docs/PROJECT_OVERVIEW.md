...
- Мессенджер: Yandex Messenger Bot API (модуль `messenger`, polling/webhook)
- БД: MySQL + Prisma
...
- Бот в Яндекс Мессенджере с текстовыми командами и кнопками под сообщениями.
...
## UI
UI реализован внутри Яндекс Мессенджера:
- Текстовые команды `/start`, `/cancel`, `/menu`; любое сообщение вне сценария показывает меню
- Кнопки под сообщениями (`suggest_buttons`) для выбора сценария и управления процессом
...
| messengerId | String (Unique) | Логин в Яндекс Мессенджере |
...
Синхронизация: все команды/кнопки обрабатываются ботом, ответы и документы отправляются через Bot API Яндекса (`YandexMessengerApiService`).
