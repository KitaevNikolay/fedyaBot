...
Бот для Яндекс Мессенджера на NestJS для взаимодействия с AI-вендором и генерации статей.
До сентября 2026 бот работал в Telegram; сейчас общение идёт через Bot API Яндекс Мессенджера
(`src/modules/messenger`), а вход в кабинет — по одноразовому коду, который бот присылает в личный чат.
...
- NestJS
- Yandex Messenger Bot API (собственный клиент на axios, polling или webhook)
- Prisma + MySQL
- Redis (ioredis)
- class-validator
...
В таблице Users сохраняются messengerId (логин Яндекса), username, firstName и lastName.
Поле role используется для доступа в кабинет. Логины из `CABINET_ADMIN_LOGINS` получают
роль admin и активный аккаунт автоматически при первом обращении к боту или входе в кабинет.
